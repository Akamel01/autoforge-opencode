...
`~/.config/opencode/autoforge/model-policy.yaml` is central authority. Hierarchy: global default (`gpt-5-nano` 400k) → role-specific (architect/planner/reviewer = `inherit` orchestrator's model; workers/discovery/validator/griller/investigator = `gpt-5-nano` 400k) → task/module → explicit runtime. Registry `model-registry.yaml` holds `context_window / max_output / capabilities / guidance` per `opencode/*` model (e.g. `muse-spark-1.2 1M`, `gpt-5/5-nano 400k`, `qwen3.5-plus 1M`, `grok-4.5 500k`). Orchestrator MUST lookup resolved model in registry before sizing each `Task`: budget = `min(context_window * 0.30, 80000)` tok; if `sum(inputs) > budget` summarize reports (paragraph + file:line) or chunk; for `fast`-only models send checklist/acceptance, for `reasoning` models include tradeoffs/risks. See Spawn contract §2b.
