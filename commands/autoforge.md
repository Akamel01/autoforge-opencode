...
For each Task you MUST: (1) resolve model via `model-policy.yaml` hierarchy (defaults → role → runtime) — `architect/planner/reviewer` = `inherit` (use orchestrator's model), workers/discovery/validator = `opencode/gpt-5-nano` (400k), (2) lookup `model-registry.yaml` for `context_window / max_output / capabilities / guidance`, (3) compute `budget = min(context_window * 0.30, 80000)` tok (e.g. `muse-spark-1.2 1M→80k cap`, `gpt-5/5-nano 400k→80k cap`), (4) shape inputs to fit budget (verbatim vs summarized vs chunked) and include `Model budget: X tok` in prompt. Architect/planner/reviewer inherit main window for critical reasoning; workers stay pinned to `gpt-5-nano` for throughput — per `model-policy.yaml`. Never stuff >budget; summarize reports to paragraph+file:line when needed. Example:

```
Task(subagent_type="autoforge-discovery", model="opencode/gpt-5-nano", description="discovery", prompt="Role: autoforge-discovery (model: opencode/gpt-5-nano per model-policy; budget 80k cap = 400k*0.30)\nModel budget: 80k tok (capabilities: tools/fast) — inputs summarized to fit\nObjective: investigate issues/tickets + repo state\nScope: repo root\nDependencies: none\nInputs: $ARGUMENTS and existing .autoforge/* if any (summarized per budget)\nConstraints: read-only, cite file:line\nRequired skills: wayfinder, find-skills\nExpected outputs: .autoforge/discovery/report.md, .autoforge/discovery/tracker-index.md\nAcceptance: tracker-index lists every frontier ticket; report cites file:line")
# planner inherits orchestrator — omit model:
# Task(subagent_type="autoforge-planner", description="plan", prompt="Role: autoforge-planner (inherits orchestrator per model-policy; budget = registry[main]*0.30 capped)\nModel budget: lookup main ...")
```
...
## First steps (always)
1. Resolve model per model-policy + lookup model-registry.yaml for window/capability/guidance; compute budget `min(context_window*0.30,80000)` and shape Task inputs accordingly (architect/planner/reviewer inherit main → lookup main window, workers `gpt-5-nano` 400k→80k cap).
